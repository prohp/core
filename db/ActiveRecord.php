...
//            'delete' => self::OP_INSERT | self::OP_UPDATE | self::OP_DELETE
    ...
    {
        $key = call_user_func([static::className(), 'tableColumns'], 'is_deleted');
...
//    }
    ...
    public static function tableColumns($cols)
    {
        ...
        $table = call_user_func([static::className(), 'tableName']);
    ...
    public static function getFields($entity)
    {
        ...
        $systemFields = \Yii::$app->configManager->findOne('db_system_fields');
        if (isset($systemFields['value'])) {
            foreach ($systemFields['value'] as $f) {
