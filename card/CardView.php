...
class CardView extends Widget
{
    ...
    public $middlewareWidget; // middleware Widget
    ...
    public function init()
    {
        ...
        // todo array_keys $this->dependencyAttributes()
        // todo регистрировать изменения таких атрибутов на клиенте
        // todo и менять frame ДОПОЛНИТЕЛЬНЫЕ АТРИБУТЫ
        $this->prepareModel();
    ...
    /**
     * Получить список кнопок для управления статусами
     * @return array
     */
    ...
    /**
     * Ключи не должны совпадать со сценариями опорной модели
     * т.к. кнопки ПОКА попадают на одни конфиг
     * @return array
     */
    ...
    {
        // TODO в JS регать на изменения события (перерисовывать карточку)
        return [
        ];
    }
    ...
    {
        if ($this->model instanceof ActiveRecordInterface && $this->model->isNewRecord) { // todo добавить EntityInterface
            return null;
        }elseif ($this->model instanceof Model && empty($this->model->id)) {
            return null;
        }
...
//        $buttons = [];
//        foreach ($configStateButtons as $name => $config) {
//            if (!is_array($config)) {
//
//            }
//        }
...
//                    'data-middleware' => $transition->getMiddleware(),
    ...
    private function renderActionButtons()
    {
        ...
        foreach ($scenarios as $scenario => $value) {
            ...
            } else { // рендрим кнопку для сценария по умолчанию
        ...
        if ($this->isFormScenario()) {
            ...
            $content = FormWidget::widget(array_merge($this->formOptions, [
                ...
                'callback' => function ($model, $form) {
                    return $this->renderDataGroups($model, $form); // $this context php 5.5+ проверить
                },
            ]));
        } else {
    ...
    private function renderDataGroups($model = null, $form = null)
    {
        ...
        $buttons = [];
        // todo наитупейший хук для перестановки местами кнопок и динамич. атрибутов,
        // todo вынести в будущем buttons в отдельный кейс в карточке
        ...
        $data = array_merge($data, ['buttons' => $buttons]); // херовый hook, надо обязательно вынести эти кнопки отсюда
        return implode('', array_map(function ($config) use ($form, $model) {
            ...
            $config = array_merge($config, [
                ...
                'form' => $form,
                'model' => $model // todo $this context php 5.5+ проверить обязательно
            ]);
